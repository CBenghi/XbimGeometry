...
namespace Xbim.Geometry.Engine.Tests
{
	...
	public class IfcModelBuilder4x3
	{
		...
		public static IfcAxis2Placement2D MakeAxis2Placement2D(MemoryModel m, double posX = 0, double posY = 0, double dirX = 1, double dirY = 0)
		{
			var p = m.Instances.New<IfcAxis2Placement2D>();
			p.RefDirection = m.Instances.New<IfcDirection>(d => d.SetXY(dirX, dirY));
			p.Location = m.Instances.New<IfcCartesianPoint>(c => c.SetXY(posX, posY));
			return p;
		}
		...
		internal static IfcPointByDistanceExpression MakePointDistanceExpression(MemoryModel m, double segm1)
		{
			var ptByDistance = m.Instances.New<IfcPointByDistanceExpression>();
			var seg1 = m.Instances.New<IfcCurveSegment>();
			seg1.Transition = IfcTransitionCode.CONTSAMEGRADIENTSAMECURVATURE;
			seg1.Placement = MakeAxis2Placement2D(m, 0.0, 0.0, 1.0, 0.0);
			seg1.SegmentStart = new IfcLengthMeasure(segm1);
			return ptByDistance;
		}

		internal static IfcGradientCurve MakePointGradientCurve(MemoryModel m)
		{
			var ptByDistance = m.Instances.New<IfcGradientCurve>();
			return ptByDistance;
		}
	}
}
